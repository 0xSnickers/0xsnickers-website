...
const FuzzyText = ({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const canvasFont = `${fontWeight} ${numericFontSize}px ${computedFontFamily}`;
      ...
      const extraWidthBuffer = Math.max(24, fuzzRange);
  ...
  const textLabel = React.Children.toArray(children).join('');

  return <canvas ref={canvasRef} aria-label={textLabel} className={className} />;
};
